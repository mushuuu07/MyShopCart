...
const products = Array.from({ length: 24 }, (_, i) => {
    ...
    return {
        ...
        desc: `The ${type} reimagined. This ShopCart exclusive features premium materials, optimized battery performance, and a sleek curved aesthetic.`
    };
});
...
async function savePaymentToMongo() {
    ...
    try {
        const response = await fetch(`${API_URL}/save-payment`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function renderSavedAddresses() {
    ...
    if (user.addresses) {
        user.addresses.forEach(a => box.innerHTML += `<div class="addr-card">${a}</div>`);
    }
}
...
function openCart() {
    ...
    cart.forEach(it => { 
        sum += it.price; 
        list.innerHTML += `<div style="padding:10px; border-bottom:1px solid #eee;"><b>${it.name}</b><br>₹${it.price.toLocaleString()}</div>`; 
    });
